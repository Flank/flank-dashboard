...
void main() {
  group("Parties", () {
    ...
    test(
      ".getParty() returns null, if the corresponding party is not found",
      () {
        ...
        expect(actualParty, isNull);
      },
    );

    test(
      ".getParty() returns the party from the parties list that accepts the config",
      () {
        when(firstParty.acceptsConfig(config)).thenReturn(false);
    ...
    test(
      ".getParty() returns the first party from the parties list that accepts the config",
      () {
