...
void main() {
  group("GithubActionsSourceValidationDelegate", () {
    ...
    test(
      ".validateAuth() returns a target validation result with an access token target if the token fetching failed",
      () async {
        ...
        expect(target, equals(GithubActionsSourceValidationTarget.accessToken));
      },
    );

    test(
      ".validateAuth() returns a target validation result with an invalid conclusion if the token fetching failed",
      () async {
        when(client.fetchToken(auth)).thenErrorWith();
    ...
    test(
      ".validateAuth() returns a target validation result with an access token target if the token fetching result is null",
      () async {
    ...
    test(
      ".validateAuth() returns a target validation result with an invalid conclusion if the token fetching result is null",
      () async {
        when(client.fetchToken(auth)).thenSuccessWith(null);
    ...
    test(
      ".validateAuth() returns a target validation result with an access token target if the fetched token does not have the required scope",
      () async {
        when(
    ...
    test(
      ".validateAuth() returns a target validation result with an invalid conclusion if the fetched token does not have the required scope",
      () async {
        when(
          client.fetchToken(auth),
        ).thenSuccessWith(const GithubToken(scopes: []));
    ...
    test(
      ".validateAuth() returns a target validation result with an access token target if the fetched token scopes are null",
      () async {
    ...
    test(
      ".validateAuth() returns a target validation result with an invalid conclusion if the fetched token scopes are null",
      () async {
        when(client.fetchToken(auth)).thenSuccessWith(const GithubToken());
    ...
    test(
      ".validateAuth() returns a target validation result with an access token target if the fetched Github token has required scope",
      () async {
    ...
    test(
      ".validateAuth() returns a target validation result with a valid conclusion if the fetched Github token has required scope",
      () async {
        when(client.fetchToken(auth)).thenSuccessWith(githubToken);
    ...
    test(
      ".validateRepositoryOwner() returns a target validation result with a repository owner target if the repository owner fetching failed",
      () async {
        ...
        expect(
          target,
          equals(GithubActionsSourceValidationTarget.repositoryOwner),
        );
      },
    );

    test(
      ".validateRepositoryOwner() returns a target validation result with an invalid conclusion if the repository owner fetching failed",
      () async {
        when(client.fetchGithubUser(repositoryOwner)).thenErrorWith();
    ...
    test(
      ".validateRepositoryOwner() returns a target validation result with a repository owner target if the repository owner fetching result is null",
      () async {
    ...
    test(
      ".validateRepositoryOwner() returns a target validation result with an invalid conclusion if the repository owner fetching result is null",
      () async {
        when(client.fetchGithubUser(repositoryOwner)).thenSuccessWith(null);
    ...
    test(
      ".validateRepositoryOwner() returns a target validation result with a repository owner target if the given repository owner is valid",
      () async {
        when(
    ...
    test(
      ".validateRepositoryOwner() returns a target validation result with a valid conclusion if the given repository owner is valid",
      () async {
        when(
          client.fetchGithubUser(repositoryOwner),
        ).thenSuccessWith(githubUser);
    ...
    test(
      ".validateRepositoryName() returns a target validation result with a repository name target if the github repository fetching failed",
      () async {
        when(
        ...
        expect(
          target,
          equals(GithubActionsSourceValidationTarget.repositoryName),
        );
      },
    );

    test(
      ".validateRepositoryName() returns a target validation result with an invalid conclusion if the github repository fetching failed",
      () async {
        when(
          client.fetchGithubRepository(
            repositoryName: repositoryName,
    ...
    test(
      ".validateRepositoryName() returns a target validation result with a repository name target if the github repository fetching result is null",
      () async {
        when(
          client.fetchGithubRepository(
            repositoryName: repositoryName,
    ...
    test(
      ".validateRepositoryName() returns a target validation result with an invalid conclusion if the github repository fetching result is null",
      () async {
        when(
          client.fetchGithubRepository(
            repositoryName: repositoryName,
    ...
    test(
      ".validateRepositoryName() returns a target validation result with a repository name target if the given repository name is valid",
      () async {
        when(
          client.fetchGithubRepository(
            repositoryName: repositoryName,
    ...
    test(
      ".validateRepositoryName() returns a target validation result with a valid conclusion if the given repository name is valid",
      () async {
        when(
          client.fetchGithubRepository(
            repositoryName: repositoryName,
    ...
    test(
      ".validateWorkflowId() returns a target validation result with a workflow identifier target if the workflow fetching failed",
      () async {
        ...
        expect(
          target,
          equals(GithubActionsSourceValidationTarget.workflowIdentifier),
        );
      },
    );

    test(
      ".validateWorkflowId() returns a target validation result with an invalid conclusion if the workflow fetching failed",
      () async {
        when(client.fetchWorkflow(workflowId)).thenErrorWith();
    ...
    test(
      ".validateWorkflowId() returns a target validation result with a workflow identifier target if the workflow fetching result is null",
      () async {
    ...
    test(
      ".validateWorkflowId() returns a target validation result with an invalid conclusion if the workflow fetching result is null",
      () async {
        when(client.fetchWorkflow(workflowId)).thenSuccessWith(null);
    ...
    test(
      ".validateWorkflowId() returns a target validation result with a workflow identifier target if the given workflow identifier is valid",
      () async {
        when(
    ...
    test(
      ".validateWorkflowId() returns a target validation result with a valid conclusion if the given workflow identifier is valid",
      () async {
        when(
          client.fetchWorkflow(workflowId),
        ).thenSuccessWith(githubActionsWorkflow);
...
////
    test(
      ".validateJobName() returns a target validation result with a job name target if the workflow runs fetching failed",
      () async {
        ...
        expect(
          target,
          equals(GithubActionsSourceValidationTarget.jobName),
        );
      },
    );

    test(
      ".validateJobName() returns a target validation result with an unknown conclusion if the workflow runs fetching failed",
      () async {
        whenFetchWorkflowRuns(workflowId).thenErrorWith();
    ...
    test(
      ".validateJobName() returns a target validation result with a job name target if the workflow runs fetching result is null",
      () async {
    ...
    test(
      ".validateJobName() returns a target validation result with an unknown conclusion if the workflow runs fetching result is null",
      () async {
        whenFetchWorkflowRuns(workflowId).thenSuccessWith(null);
    ...
    test(
      ".validateJobName() returns a target validation result with a job name target if there are no completed workflow runs",
      () async {
    ...
    test(
      ".validateJobName() returns a target validation result with an unknown conclusion if there are no completed workflow runs",
      () async {
        whenFetchWorkflowRuns(workflowId).thenSuccessWith(emptyRunsPage);
    ...
    test(
      ".validateJobName() returns a target validation result with a job name target if the workflow run jobs page fetching failed",
      () async {
    ...
    test(
      ".validateJobName() returns a target validation result with an unknown conclusion if the workflow run jobs page fetching failed",
      () async {
        whenFetchRunJobs().thenErrorWith();
    ...
    test(
      ".validateJobName() returns a target validation result with a job name target if the workflow run jobs page fetching result is null",
      () async {
    ...
    test(
      ".validateJobName() returns a target validation result with an unknown conclusion if the workflow run jobs page fetching result is null",
      () async {
        whenFetchRunJobs().thenSuccessWith(null);
    ...
    test(
      ".validateJobName() returns a target validation result with a job name target if the next workflow run jobs page fetching failed",
      () async {
    ...
    test(
      ".validateJobName() returns a target validation result with an unknown conclusion if the next workflow run jobs page fetching failed",
      () async {
        whenFetchRunJobs().thenSuccessWith(workflowRunsPageHasNext);
        when(
          client.fetchRunJobsNext(workflowRunsPageHasNext),
    ...
    test(
      ".validateJobName() returns a target validation result with a job name target if the next workflow run jobs page fetching result is null",
      () async {
        whenFetchRunJobs().thenSuccessWith(workflowRunsPageHasNext);
        when(
          client.fetchRunJobsNext(workflowRunsPageHasNext),
    ...
    test(
      ".validateJobName() returns a target validation result with an unknown conclusion if the next workflow run jobs page fetching result is null",
      () async {
        whenFetchRunJobs().thenSuccessWith(workflowRunsPageHasNext);
        when(
          client.fetchRunJobsNext(workflowRunsPageHasNext),
    ...
    test(
      ".validateJobName() returns a target validation result with a job name target if there is no job with the given job name",
      () async {
    ...
    test(
      ".validateJobName() returns a target validation result with an invalid conclusion if there is no job with the given job name",
      () async {
        whenFetchRunJobs().thenSuccessWith(emptyJobsPage);
    ...
    test(
      ".validateJobName() returns a target validation result with a job name target if the given job name is valid",
      () async {
        whenFetchRunJobs().thenSuccessWith(jobsPage);
    ...
    test(
      ".validateJobName() returns a target validation result with a valid conclusion if the given job name is valid",
      () async {
        whenFetchRunJobs().thenSuccessWith(jobsPage);
...
///////////////////////
    test(
      ".validateCoverageArtifactName() returns a target validation result with a coverage artifact name target if the workflow runs fetching failed",
      () async {
        ...
        expect(
          target,
          equals(GithubActionsSourceValidationTarget.coverageArtifactName),
        );
      },
    );

    test(
      ".validateCoverageArtifactName() returns a target validation result with an unknown conclusion if the workflow runs fetching failed",
      () async {
        whenFetchWorkflowRunsWithConclusion(workflowId).thenErrorWith();
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with a coverage artifact name target if the successful workflow runs fetching result is null",
      () async {
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with an unknown conclusion if the successful workflow runs fetching result is null",
      () async {
        whenFetchWorkflowRunsWithConclusion(workflowId).thenSuccessWith(null);
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with a coverage artifact name target if there are no successful workflow runs",
      () async {
        whenFetchWorkflowRunsWithConclusion(
          workflowId,
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with an unknown conclusion if there are no successful workflow runs",
      () async {
        whenFetchWorkflowRunsWithConclusion(
          workflowId,
        ).thenSuccessWith(emptyRunsPage);
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with a coverage artifact name target if the workflow run artifacts page fetching failed",
      () async {
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with an unknown conclusion if the workflow run artifacts page fetching failed",
      () async {
        whenFetchRunArtifacts().thenErrorWith();
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with a coverage artifact name target if the successful workflow run artifacts page fetching result is null",
      () async {
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with an unknown conclusion if the successful workflow run artifacts page fetching result is null",
      () async {
        whenFetchRunArtifacts().thenSuccessWith(null);
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with a coverage artifact name target if the next workflow run artifacts page fetching failed",
      () async {
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with an unknown conclusion if the next workflow run artifacts page fetching failed",
      () async {
        whenFetchRunArtifacts().thenSuccessWith(artifactsPageHasNext);
        when(
          client.fetchRunArtifactsNext(artifactsPageHasNext),
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with a coverage artifact name target if the successful next workflow run artifacts page fetching result is null",
      () async {
        whenFetchRunArtifacts().thenSuccessWith(artifactsPageHasNext);
        when(
          client.fetchRunArtifactsNext(artifactsPageHasNext),
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with an unknown conclusion if the successful next workflow run artifacts page fetching result is null",
      () async {
        whenFetchRunArtifacts().thenSuccessWith(artifactsPageHasNext);
        when(
          client.fetchRunArtifactsNext(artifactsPageHasNext),
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with a coverage artifact name target if there is no artifact with the given coverage artifact name",
      () async {
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with an invalid conclusion if there is no artifact with the given coverage artifact name",
      () async {
        whenFetchRunArtifacts().thenSuccessWith(emptyArtifactsPage);
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with a coverage artifact name target if the given coverage artifact name is valid",
      () async {
        whenFetchRunArtifacts().thenSuccessWith(artifactsPage);
    ...
    test(
      ".validateCoverageArtifactName() returns a target validation result with a valid conclusion if the given coverage artifact name is valid",
      () async {
        whenFetchRunArtifacts().thenSuccessWith(artifactsPage);
